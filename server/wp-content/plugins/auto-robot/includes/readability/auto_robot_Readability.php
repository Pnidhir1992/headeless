...
require_once(dirname(__FILE__).'/auto_robot_JSLikeHTMLElement.php');

class auto_robot_Readability
{
	...
	/**
	* All of the regular expressions in use within auto_robot_Readability.
	...
	**/
	...
	/**
	* Create instance of auto_robot_Readability
	...
	*/
	function __construct($html, $url=null, $parser='libxml')
	{
		...
		$this->dom->registerNodeClass('DOMElement', 'auto_robot_JSLikeHTMLElement');
	}
	...
	*/
	...
	/**
	* Runs auto_robot_Readability.
	*
	* Workflow:
	*  1. Prep the document by removing script tags, css, etc.
	*  2. Build auto_robot_Readability's DOM tree.
	...
	**/
	public function init()
	{
		...
		/* Build auto_robot_Readability's DOM tree */
		...
		if (!$articleContent) {
			...
			$articleContent->setAttribute('id', 'auto_robot_Readability-content');
			$articleContent->innerHTML = '<p>Sorry, auto_robot_Readability was unable to parse this page for content.</p>';
		}
	...
	*/
	...
	/**
	* Prepare the HTML document for auto_robot_Readability to scrape it.
	...
	**/
	protected function prepDocument() {
		...
		*/
		...
		$this->body->setAttribute('id', 'auto_robot_ReadabilityBody');
		...
		//document.body.innerHTML = document.body.innerHTML.replace(auto_robot_Readability.regexps.replaceBrs, '</p><p>').replace(auto_robot_Readability.regexps.replaceFonts, '<$1span>');
	...
	**/
	public function addFootnotes($articleContent) {
		...
		$footnotesWrapper->setAttribute('id', 'auto_robot_Readability-footnotes');
		$footnotesWrapper->innerHTML = '<h3>References</h3>';

		$articleFootnotes = $this->dom->createElement('ol');
		$articleFootnotes->setAttribute('id', 'auto_robot_Readability-footnotes-list');
		...
		for ($i = 0; $i < $articleLinks->length; $i++)
		{
			...
			if ((strpos($articleLink->getAttribute('class'), 'auto_robot_Readability-DoNotFootnote') !== false) || preg_match($this->regexps['skipFootnoteLink'], $linkText)) {
				continue;
			}
			...
			$refLink->setAttribute('href', '#auto_robot_ReadabilityFootnoteLink-' . $linkCount);
			$refLink->innerHTML = '<small><sup>[' . $linkCount . ']</sup></small>';
			$refLink->setAttribute('class', 'auto_robot_Readability-DoNotFootnote');
			...
			$articleLink->setAttribute('name', 'auto_robot_ReadabilityLink-' . $linkCount);

			$footnote->innerHTML = '<small><sup><a href="#auto_robot_ReadabilityLink-' . $linkCount . '" title="Jump to Link in Article">^</a></sup></small> ';

			$footnoteLink->innerHTML = ($footnoteLink->getAttribute('title') != '' ? $footnoteLink->getAttribute('title') : $linkText);
			$footnoteLink->setAttribute('name', 'auto_robot_ReadabilityFootnoteLink-' . $linkCount);
	...
	/**
	* Reverts P elements with class 'auto_robot_Readability-styled'
	* to text nodes - which is what they were before.
	*
	* @param DOMElement
	* @return void
	*/
	function revertauto_robot_ReadabilityStyledElements($articleContent) {
		$xpath = new DOMXPath($articleContent->ownerDocument);
		$elems = $xpath->query('.//p[@class="auto_robot_Readability-styled"]', $articleContent);
		//$elems = $articleContent->getElementsByTagName('p');
		for ($i = $elems->length-1; $i >= 0; $i--) {
			$e = $elems->item($i);
			$e->parentNode->replaceChild($articleContent->ownerDocument->createTextNode($e->textContent), $e);
			//if ($e->hasAttribute('class') && $e->getAttribute('class') == 'auto_robot_Readability-styled') {
	...
	*/
	function prepArticle($articleContent) {
		...
		if ($this->revertForcedParagraphElements) {
			$this->revertauto_robot_ReadabilityStyledElements($articleContent);
		}
	...
	/**
	* Initialize a node with the auto_robot_Readability object. Also checks the
	...
	**/
	protected function initializeNode($node) {
		$auto_robot_Readability = $this->dom->createAttribute('auto_robot_Readability');
		$auto_robot_Readability->value = 0; // this is our contentScore
		$node->setAttributeNode($auto_robot_Readability);

		switch (strtoupper($node->tagName)) { // unsure if strtoupper is needed, but using it just in case
			case 'DIV':
				$auto_robot_Readability->value += 5;
				break;

			case 'PRE':
			case 'TD':
			case 'BLOCKQUOTE':
				$auto_robot_Readability->value += 3;
				break;
		...
		$auto_robot_Readability->value += $this->getClassWeight($node);
	}
	...
	protected function grabArticle($page=null) {
				...
				else
				{
					...
					for ($i = 0, $il = $node->childNodes->length; $i < $il; $i++) {
						...
						if ($childNode->nodeType == 3) { // XML_TEXT_NODE
							...
							$p->setAttribute('class', 'auto_robot_Readability-styled');
		...
		**/
		...
		for ($pt=0; $pt < count($nodesToScore); $pt++) {
			...
			/* Initialize auto_robot_Readability data for the parent. */
			if (!$parentNode->hasAttribute('auto_robot_Readability'))
			{
				$this->initializeNode($parentNode);
				$candidates[] = $parentNode;
			}

			/* Initialize auto_robot_Readability data for the grandparent. */
			if ($grandParentNode && !$grandParentNode->hasAttribute('auto_robot_Readability') && isset($grandParentNode->tagName))
			{
			...
			$parentNode->getAttributeNode('auto_robot_Readability')->value += $contentScore;

			if ($grandParentNode) {
				$grandParentNode->getAttributeNode('auto_robot_Readability')->value += $contentScore/2;
			}
		}
		...
		**/
		...
		for ($c=0, $cl=count($candidates); $c < $cl; $c++)
		{
			...
			**/
			$auto_robot_Readability = $candidates[$c]->getAttributeNode('auto_robot_Readability');
			$auto_robot_Readability->value = $auto_robot_Readability->value * (1-$this->getLinkDensity($candidates[$c]));

			$this->dbg('Candidate: ' . $candidates[$c]->tagName . ' (' . $candidates[$c]->getAttribute('class') . ':' . $candidates[$c]->getAttribute('id') . ') with score ' . $auto_robot_Readability->value);

			if (!$topCandidate || $auto_robot_Readability->value > (int)$topCandidate->getAttribute('auto_robot_Readability')) {
		...
		**/
		$articleContent        = $this->dom->createElement('div');
		@$articleContent->setAttribute('id', 'auto_robot_Readability-content');
		$siblingScoreThreshold = max(10, ((int)$topCandidate->getAttribute('auto_robot_Readability')) * 0.2);
		...
		for ($s=0, $sl=$siblingNodes->length; $s < $sl; $s++)
		{
			...
			$this->dbg('Looking at sibling node: ' . $siblingNode->nodeName . (($siblingNode->nodeType === XML_ELEMENT_NODE && $siblingNode->hasAttribute('auto_robot_Readability')) ? (' with score ' . $siblingNode->getAttribute('auto_robot_Readability')) : ''));

			//dbg('Sibling has score ' . ($siblingNode->auto_robot_Readability ? siblingNode.auto_robot_Readability.contentScore : 'Unknown'));
			...
			if ($siblingNode->nodeType === XML_ELEMENT_NODE && $siblingNode->getAttribute('class') == $topCandidate->getAttribute('class') && $topCandidate->getAttribute('class') != '') {
				$contentBonus += ((int)$topCandidate->getAttribute('auto_robot_Readability')) * 0.2;
			}

			if ($siblingNode->nodeType === XML_ELEMENT_NODE && $siblingNode->hasAttribute('auto_robot_Readability') && (((int)$siblingNode->getAttribute('auto_robot_Readability')) + $contentBonus) >= $siblingScoreThreshold)
			{
				$append = true;
			}
			...
			if ($append)
			{
				...
				/* To ensure a node does not interfere with auto_robot_Readability styles, remove its classnames */
	...
	public function cleanConditionally($e, $tag) {
		...
		*/
		for ($i=$curTagsLength-1; $i >= 0; $i--) {
			...
			$contentScore = ($tagsList->item($i)->hasAttribute('auto_robot_Readability')) ? (int)$tagsList->item($i)->getAttribute('auto_robot_Readability') : 0;

			$this->dbg('Cleaning Conditionally ' . $tagsList->item($i)->tagName . ' (' . $tagsList->item($i)->getAttribute('class') . ':' . $tagsList->item($i)->getAttribute('id') . ')' . (($tagsList->item($i)->hasAttribute('auto_robot_Readability')) ? (' with score ' . $tagsList->item($i)->getAttribute('auto_robot_Readability')) : ''));
